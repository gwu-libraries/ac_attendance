...
process_attendance <- function(roster_path, attendance_path, selected_session, max_weekly_points) {
  ...
  attendance <- attendance_raw %>%
    ...
    mutate(session_date = parse_date_time(session_date, orders = c('ymd', 'mdy')), # because Penji keeps changing the format
           ...
           week_num = epiweek(session_date - days(1)), # week of the calendar year
  ...
  sessions <- sessions_raw %>%
    mutate(start_date = ymd(start_date),
           ...
           start_week = epiweek(start_date- days(1)), # week of the calendar year
           end_week = epiweek(end_date - days(1)),    # week of the calendar year
  ...
  attendance_stats <- attendance_stats %>%
    left_join(sessions %>% select(year, start_date, start_week, end_week, session_label),
              join_by(session_year == year,
                      week_num >= start_week,         # limit to info from the session corresponding to the attendance week
                      week_num <= end_week)) %>%
    filter(session_year == selected_session_year) %>%
    filter(session_term == selected_session_term_code) %>%
    mutate(session_week = week_num - start_week + 1,  # week number within the session (starting with 1)
           week_start = start_date + weeks(session_week - 1),
           week_end = start_date + weeks(session_week - 1) + 6,
...
create_zip <- function(attendance_stats, zip_filename) {
  ...
  xlsx_basename <- paste0("attendance_", format(Sys.Date(), "%Y%m%d"), "_", format(Sys.time(), "%H%M"), ".xlsx")
  ...
  for (course_i in unique(attendance_stats$course)) {
    course_df <- attendance_stats %>%
      ...
      summarize(total_sessions = sum(n_sessions),
                attendance_points = sum(weekly_attendance_points)) %>%
      arrange(section_number, student_lastname, student_firstname)
    xlsx_i_basename <- xlsx_basename <- paste0("attendance_summary_", course_i, "_", format(Sys.Date(), "%Y%m%d"), ".xlsx")
